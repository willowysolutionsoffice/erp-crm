...
export default function PendingJobOrdersPage() {
  ...
  return (
    <div className="@container/main flex flex-1 flex-col gap-6 p-4 md:p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Pending Job Orders</h1>
          <p className="text-gray-600">View job orders with pending actions</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {pagination
              ? `Total: ${pagination.total} pending job orders${search ? ` (${filteredJobOrders.length} filtered)` : ''}`
              : 'Loading...'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="flex items-center justify-center p-8">
              <div className="text-muted-foreground">
                {search ? 'No job orders match your search' : 'No pending job orders found'}
              </div>
            </div>
          ) : isMobile ? (
